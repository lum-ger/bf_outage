...
# --- Configuration --- (Same as before, but removed CSV output)
...
# --- Time Series Creation --- (Same as before)
...
# Black Friday spike
df['traffic'] = np.where(
    (df['timestamp'].dt.date == datetime.datetime(2024, 11, 29).date()),  # Corrected line
...
df['traffic'] = df['traffic'].clip(lower=0).astype(int)  # Corrected line: Ensure no negative traffic

# --- Revenue Calculation ---
df['revenue'] = df['traffic'] * REVENUE_PER_REQUEST
df['revenue'] = df['revenue'].clip(lower=0).astype(int) # Corrected line
